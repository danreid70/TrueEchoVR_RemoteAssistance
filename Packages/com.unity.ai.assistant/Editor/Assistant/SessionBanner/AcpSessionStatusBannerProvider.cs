...
namespace Unity.AI.Assistant.Editor.SessionBanner
{
    class AcpSessionStatusBannerProvider
    {
        ...
        public VisualElement GetCurrentView()
        {
            ...
            switch (ProviderStateObserver.ReadyState)
            {
                ...
                case ProviderStateObserver.ProviderReadyState.Error:
                    ...
                    if (errorCode == AcpConstants.ErrorCode_GatewayCapReached)
                    {
                        // Render without the generic "Failed to start <provider>
                        // session." framing — the tier-denial copy is already
                        // self-contained and the "failure" framing misleads
                        // users into thinking something went wrong rather than
                        // their plan simply not including this feature.
                        return BuildCapReachedBanner();
                    }
        ...
        BasicBannerContent BuildCapReachedBanner()
        {
            // The ProviderStateObserver error message was built by
            // TierDenial and already contains the full "Your plan doesn't
            // include X" + "Upgrade your Unity plan" copy, so we pass it
            // straight through rather than re-composing it here. Rendered
            // with the info icon so users read it as "this is the policy,"
            // not "something went wrong and needs triage."
            var message = ProviderStateObserver.InitializationError;
            if (string.IsNullOrEmpty(message))
                message = "Your Unity plan doesn't include AI Gateway sessions. Upgrade your Unity plan to add more.";
            return new BasicBannerContent(message, links: null, useInfoIcon: true);
        }
