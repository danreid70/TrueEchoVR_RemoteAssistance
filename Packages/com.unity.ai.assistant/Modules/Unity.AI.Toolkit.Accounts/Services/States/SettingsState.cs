...
namespace Unity.AI.Toolkit.Accounts.Services.States
{
    class SettingsState : IDisposable
    {
        ...
        // When the backend has spoken, its value is authoritative — including 0.
        // Under the post-2026-04 business model, 0 is a legitimate entitlement
        // ("free tier: no MCP/gateway connections"), not a missing-field
        // sentinel. Collapsing 0 to 1 would silently grant a free slot to
        // users who aren't entitled to any.
        //
        // We fail closed (return 0) during the transient loading window where
        // Value is null, because that window is sub-second on editor startup /
        // domain reload (AcpEntitlementWiring.Apply fires on Account.settings
        // .OnChange so the real caps are installed as soon as GetSettings
        // returns). Rejecting connection attempts during that window is the
        // safer default than letting free users pre-open a slot before their
        // entitlement is known. -1 remains "unlimited" (Enterprise).
        public int AllowedGatewayConnections => Value?.AllowedGatewayConnections ?? 0;
        public int AllowedMcpConnections => Value?.AllowedMcpConnections ?? 0;
