...
-   **Single point-at driver (v2.5):** All "point at a code" visuals — the green holographic focus box (`QrCodeManager.EnsureFocusGlow`/`ApplyFocusGlowPulse`), the faint dashed line (`VrHudController.UpdateDashLine` / `PointAtDashLine`), the HUD arrow, the dropdown selection, and the HUD message — are set by **`SessionUiController.ApplyPointTarget(QRCodeInstance)`**. The dropdown, remote `point-to`, and "stop pointing" all route through it; the dropdown value is synced with `SetValueWithoutNotify`. Don't point/clear any of these visuals independently — go through `ApplyPointTarget` (passing `null` clears). Selecting the dropdown's first item (the prompt) clears.
-   **RoomAnchor live-follow vs. spatial-anchor pin (v2.5):** when `roomAnchorVisualFollowsLiveQr` is true (default), the RoomAnchor visual snaps to the live QR pose **only while the RoomAnchor code is actually tracked**, and the persisted `OVRSpatialAnchor` holds it otherwise. The RoomAnchor uses its own tighter thresholds (`roomAnchorPositionThreshold` ≈ 4 mm, `roomAnchorRotationThreshold` ≈ 0.1°). Backend coordinate sync is unchanged. Set the flag false to keep the spatial anchor fully authoritative.
-   **Sign-in gating (v2.5):** the app always opens on the Login panel and the **Sign In button stays disabled until credentials exist** (persisted/just-scanned setup code or a valid resolve). The raw Customer/Location ID input fields are hidden — they're provisioned by the setup code, not typed.
-   **Sign-in QR re-detection suppression (v2.5):** the reconcile sweep re-processed the setup QR every ~0.5 s during SignIn, spamming chat and wasting work. The setup code is now added to `QrCodeManager._ignoredSessionTrackables` after its first sighting in a session (cleared on sign-out). Chat also collapses consecutive duplicate lines into `… (xN)`.
-   **Push/Pull button state (v2.5):** `SessionUiController.UpdateSessionButtonsState()` enables **Pull** only with a known Location and **Push** only with a Location plus ≥1 uploadable code (so an empty/anchor-less push can't silently no-op). Call it after any change that affects uploadability (QR add/remove, anchor discovered, clear, detection toggle).